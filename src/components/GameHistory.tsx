    ...
    <div key={entry.id} className="border-b border-border/20 pb-3 mb-3 last:border-b-0">
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-2">
          <span className="text-lg">{getResultIcon(entry.result_type)}</span>
          <div>
            <span className="font-medium capitalize">{entry.game_type.replace('-', ' ')}</span>
            <p className="text-xs text-muted-foreground">{formatDate(entry.created_at)}</p>
          </div>
        </div>
        <Badge variant="secondary" className={getResultColor(entry.result_type)}>
          {entry.result_type.toUpperCase()}
        </Badge>
      </div>
